...
class EDecoder implements ObjectInput {
	...
	private void processSecurityDefinitionOptionalParameterMsg() throws IOException {
		...
		m_EWrapper.securityDefinitionOptionalParameter(reqId, exchange, underlyingConId, tradingClass, multiplier,
				expirations, strikes);
	}
	...
	private void processMarketDepthL2Msg() throws IOException {
		...
		m_EWrapper.updateMktDepthL2(id, position, marketMaker, operation, side, price, size);
	}
	...
	private void processMarketDepthMsg() throws IOException {
		...
		m_EWrapper.updateMktDepth(id, position, operation, side, price, size);
	}
	...
	private void processScannerDataMsg() throws IOException {
		...
		for (int ctr = 0; ctr < numberOfElements; ctr++) {
			...
			m_EWrapper.scannerData(tickerId, rank, contract, distance, benchmark, projection, legsStr);
		}
	...
	private void processPortfolioValueMsg() throws IOException {
		...
		m_EWrapper.updatePortfolio(contract, position, marketPrice, marketValue, averageCost, unrealizedPNL,
	...
	private void processOrderStatusMsg() throws IOException {
		...
		m_EWrapper.orderStatus(id, status, filled, remaining, avgFillPrice, permId, parentId, lastFillPrice, clientId,
				whyHeld);
	}
	...
	private void processTickEFPMsg() throws IOException {
		...
		m_EWrapper.tickEFP(tickerId, tickType, basisPoints, formattedBasisPoints, impliedFuturesPrice, holdDays,
	...
	private void processTickOptionComputatioMsg() throws IOException {
		...
		m_EWrapper.tickOptionComputation(tickerId, tickType, impliedVol, delta, optPrice, pvDividend, gamma, vega,
				theta, undPrice);
	}
