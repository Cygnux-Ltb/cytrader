...
public class Order {
	...
	private int m_parentId; // Parent order Id, to associate Auto STP or TRAIL orders with the original
							// order.
	...
	private double m_trailingPercent = Double.MAX_VALUE; // for Trailing Stop orders; specify the percentage, e.g. 3,
															// not .03
	...
	private int m_shortSaleSlot; // 1 if you hold the shares, 2 if they will be delivered from elsewhere. Only
									// for Action="SSHORT
	...
	public boolean equals(Object p_other) {
		...
		if (m_orderId != l_theOther.m_orderId || m_clientId != l_theOther.m_clientId
				...
				|| m_displaySize != l_theOther.m_displaySize || m_triggerMethod != l_theOther.m_triggerMethod
				...
				|| m_percentOffset != l_theOther.m_percentOffset || m_trailStopPrice != l_theOther.m_trailStopPrice
				|| m_trailingPercent != l_theOther.m_trailingPercent || m_origin != l_theOther.m_origin
				|| m_shortSaleSlot != l_theOther.m_shortSaleSlot || m_discretionaryAmt != l_theOther.m_discretionaryAmt
				...
				|| m_auctionStrategy != l_theOther.m_auctionStrategy || m_startingPrice != l_theOther.m_startingPrice
				|| m_stockRefPrice != l_theOther.m_stockRefPrice || m_delta != l_theOther.m_delta
				|| m_stockRangeLower != l_theOther.m_stockRangeLower
				|| m_stockRangeUpper != l_theOther.m_stockRangeUpper || m_volatility != l_theOther.m_volatility
				...
				|| m_basisPoints != l_theOther.m_basisPoints || m_basisPointsType != l_theOther.m_basisPointsType
				...
				|| m_scaleRandomPercent != l_theOther.m_scaleRandomPercent || m_whatIf != l_theOther.m_whatIf
				|| m_notHeld != l_theOther.m_notHeld || m_exemptCode != l_theOther.m_exemptCode
				|| m_randomizePrice != l_theOther.m_randomizePrice || m_randomizeSize != l_theOther.m_randomizeSize
				|| m_solicited != l_theOther.m_solicited || m_referenceContractId != l_theOther.m_referenceContractId
				|| m_peggedChangeAmount != l_theOther.m_peggedChangeAmount
				|| m_referenceChangeAmount != l_theOther.m_referenceChangeAmount
				|| m_adjustedOrderType != l_theOther.m_adjustedOrderType || m_triggerPrice != l_theOther.m_triggerPrice
