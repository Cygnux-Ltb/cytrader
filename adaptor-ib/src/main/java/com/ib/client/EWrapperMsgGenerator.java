...
public class EWrapperMsgGenerator {
	...
	static public String tickOptionComputation(int tickerId, int field, double impliedVol, double delta,
			double optPrice, double pvDividend, double gamma, double vega, double theta, double undPrice) {
		String toAdd = "id=" + tickerId + "  " + TickType.getField(field) + ": vol = "
				+ ((impliedVol >= 0 && impliedVol != Double.MAX_VALUE) ? Double.toString(impliedVol) : "N/A")
				+ " delta = " + ((Math.abs(delta) <= 1) ? Double.toString(delta) : "N/A") + " gamma = "
				+ ((Math.abs(gamma) <= 1) ? Double.toString(gamma) : "N/A") + " vega = "
				+ ((Math.abs(vega) <= 1) ? Double.toString(vega) : "N/A") + " theta = "
				+ ((Math.abs(theta) <= 1) ? Double.toString(theta) : "N/A") + " optPrice = "
				+ ((optPrice >= 0 && optPrice != Double.MAX_VALUE) ? Double.toString(optPrice) : "N/A")
				+ " pvDividend = "
				+ ((pvDividend >= 0 && pvDividend != Double.MAX_VALUE) ? Double.toString(pvDividend) : "N/A")
				+ " undPrice = "
	...
	static public String tickEFP(int tickerId, int tickType, double basisPoints, String formattedBasisPoints,
	...
	static public String orderStatus(int orderId, String status, double filled, double remaining, double avgFillPrice,
			int permId, int parentId, double lastFillPrice, int clientId, String whyHeld) {
		return "order status: orderId=" + orderId + " clientId=" + clientId + " permId=" + permId + " status=" + status
				+ " filled=" + filled + " remaining=" + remaining + " avgFillPrice=" + avgFillPrice + " lastFillPrice="
				+ lastFillPrice + " parent Id=" + parentId + " whyHeld=" + whyHeld;
	}

	static public String openOrder(int orderId, Contract contract, Order order, OrderState orderState) {
		String msg = "open order: orderId=" + orderId + " action=" + order.getAction() + " quantity="
				+ order.totalQuantity() + " conid=" + contract.conid() + " symbol=" + contract.symbol() + " secType="
				+ contract.getSecType() + " lastTradeDate=" + contract.lastTradeDateOrContractMonth() + " strike="
				+ contract.strike() + " right=" + contract.getRight() + " multiplier=" + contract.multiplier()
				+ " exchange=" + contract.exchange() + " primaryExch=" + contract.primaryExch() + " currency="
				+ contract.currency() + " localSymbol=" + contract.localSymbol() + " tradingClass="
				+ contract.tradingClass() + " type=" + order.getOrderType() + " lmtPrice="
				+ Util.DoubleMaxString(order.lmtPrice()) + " auxPrice=" + Util.DoubleMaxString(order.auxPrice())
				+ " TIF=" + order.getTif() + " localSymbol=" + contract.localSymbol() + " client Id=" + order.clientId()
				+ " parent Id=" + order.parentId() + " permId=" + order.permId() + " outsideRth=" + order.outsideRth()
				+ " hidden=" + order.hidden() + " discretionaryAmt=" + order.discretionaryAmt() + " displaySize="
				+ order.displaySize() + " triggerMethod=" + order.getTriggerMethod() + " goodAfterTime="
				+ order.goodAfterTime() + " goodTillDate=" + order.goodTillDate() + " faGroup=" + order.faGroup()
				+ " faMethod=" + order.getFaMethod() + " faPercentage=" + order.faPercentage() + " faProfile="
				+ order.faProfile() + " shortSaleSlot=" + order.shortSaleSlot() + " designatedLocation="
				+ order.designatedLocation() + " exemptCode=" + order.exemptCode() + " ocaGroup=" + order.ocaGroup()
				+ " ocaType=" + order.getOcaType() + " rule80A=" + order.getRule80A() + " allOrNone="
				+ order.allOrNone() + " minQty=" + Util.IntMaxString(order.minQty()) + " percentOffset="
				+ Util.DoubleMaxString(order.percentOffset()) + " eTradeOnly=" + order.eTradeOnly() + " firmQuoteOnly="
				+ order.firmQuoteOnly() + " nbboPriceCap=" + Util.DoubleMaxString(order.nbboPriceCap())
				+ " optOutSmartRouting=" + order.optOutSmartRouting() + " auctionStrategy=" + order.auctionStrategy()
				+ " startingPrice=" + Util.DoubleMaxString(order.startingPrice()) + " stockRefPrice="
				+ Util.DoubleMaxString(order.stockRefPrice()) + " delta=" + Util.DoubleMaxString(order.delta())
				+ " stockRangeLower=" + Util.DoubleMaxString(order.stockRangeLower()) + " stockRangeUpper="
				+ Util.DoubleMaxString(order.stockRangeUpper()) + " volatility="
				+ Util.DoubleMaxString(order.volatility()) + " volatilityType=" + order.getVolatilityType()
				+ " deltaNeutralOrderType=" + order.getDeltaNeutralOrderType() + " deltaNeutralAuxPrice="
				+ Util.DoubleMaxString(order.deltaNeutralAuxPrice()) + " deltaNeutralConId=" + order.deltaNeutralConId()
				+ " deltaNeutralSettlingFirm=" + order.deltaNeutralSettlingFirm() + " deltaNeutralClearingAccount="
				+ order.deltaNeutralClearingAccount() + " deltaNeutralClearingIntent="
				+ order.deltaNeutralClearingIntent() + " deltaNeutralOpenClose=" + order.deltaNeutralOpenClose()
				+ " deltaNeutralShortSale=" + order.deltaNeutralShortSale() + " deltaNeutralShortSaleSlot="
				+ order.deltaNeutralShortSaleSlot() + " deltaNeutralDesignatedLocation="
				+ order.deltaNeutralDesignatedLocation() + " continuousUpdate=" + order.continuousUpdate()
				+ " referencePriceType=" + order.getReferencePriceType() + " trailStopPrice="
				+ Util.DoubleMaxString(order.trailStopPrice()) + " trailingPercent="
				+ Util.DoubleMaxString(order.trailingPercent()) + " scaleInitLevelSize="
				+ Util.IntMaxString(order.scaleInitLevelSize()) + " scaleSubsLevelSize="
				+ Util.IntMaxString(order.scaleSubsLevelSize()) + " scalePriceIncrement="
				+ Util.DoubleMaxString(order.scalePriceIncrement()) + " scalePriceAdjustValue="
				+ Util.DoubleMaxString(order.scalePriceAdjustValue()) + " scalePriceAdjustInterval="
				+ Util.IntMaxString(order.scalePriceAdjustInterval()) + " scaleProfitOffset="
				+ Util.DoubleMaxString(order.scaleProfitOffset()) + " scaleAutoReset=" + order.scaleAutoReset()
				+ " scaleInitPosition=" + Util.IntMaxString(order.scaleInitPosition()) + " scaleInitFillQty="
				+ Util.IntMaxString(order.scaleInitFillQty()) + " scaleRandomPercent=" + order.scaleRandomPercent()
				+ " hedgeType=" + order.getHedgeType() + " hedgeParam=" + order.hedgeParam() + " account="
				+ order.account() + " modelCode = " + order.modelCode() + " settlingFirm=" + order.settlingFirm()
				+ " clearingAccount=" + order.clearingAccount() + " clearingIntent=" + order.clearingIntent()
				+ " notHeld=" + order.notHeld() + " whatIf=" + order.whatIf() + " solicited=" + order.solicited()
				+ " randomize size=" + order.randomizeSize() + " randomize price=" + order.randomizePrice();
		...
		if (contract.underComp() != null) {
			...
			msg += " underComp.conId =" + underComp.conid() + " underComp.delta =" + underComp.delta()
		...
		String orderStateMsg = " status=" + orderState.getStatus() + " initMargin=" + orderState.initMargin()
				+ " maintMargin=" + orderState.maintMargin() + " equityWithLoan=" + orderState.equityWithLoan()
				+ " commission=" + Util.DoubleMaxString(orderState.commission()) + " minCommission="
				+ Util.DoubleMaxString(orderState.minCommission()) + " maxCommission="
				+ Util.DoubleMaxString(orderState.maxCommission()) + " commissionCurrency="
				+ orderState.commissionCurrency() + " warningText=" + orderState.warningText();
	...
	static public String updatePortfolio(Contract contract, int position, double marketPrice, double marketValue,
			double averageCost, double unrealizedPNL, double realizedPNL, String accountName) {
		String msg = "updatePortfolio: " + contractMsg(contract) + position + " " + marketPrice + " " + marketValue
	...
	static public String updateMktDepthL2(int tickerId, int position, String marketMaker, int operation, int side,
			double price, int size) {
		return "updateMktDepth: " + tickerId + " " + position + " " + marketMaker + " " + operation + " " + side + " "
				+ price + " " + size;
	}
	...
	static public String historicalData(int reqId, String date, double open, double high, double low, double close,
			int volume, int count, double WAP, boolean hasGaps) {
		return "id=" + reqId + " date = " + date + " open=" + open + " high=" + high + " low=" + low + " close=" + close
				+ " volume=" + volume + " count=" + count + " WAP=" + WAP + " hasGaps=" + hasGaps;
	}

	public static String realtimeBar(int reqId, long time, double open, double high, double low, double close,
			long volume, double wap, int count) {
		return "id=" + reqId + " time = " + time + " open=" + open + " high=" + high + " low=" + low + " close=" + close
	...
	static public String scannerData(int reqId, int rank, ContractDetails contractDetails, String distance,
			String benchmark, String projection, String legsStr) {
		Contract contract = contractDetails.contract();
		return "id = " + reqId + " rank=" + rank + " symbol=" + contract.symbol() + " secType=" + contract.getSecType()
				+ " lastTradeDate=" + contract.lastTradeDateOrContractMonth() + " strike=" + contract.strike()
				+ " right=" + contract.getRight() + " exchange=" + contract.exchange() + " currency="
				+ contract.currency() + " localSymbol=" + contract.localSymbol() + " marketName="
				+ contractDetails.marketName() + " tradingClass=" + contract.tradingClass() + " distance=" + distance
	...
	static public String deltaNeutralValidation(int reqId, DeltaNeutralContract underComp) {
		return "id = " + reqId + " underComp.conId =" + underComp.conid() + " underComp.delta =" + underComp.delta()
				+ " underComp.price =" + underComp.price();
	}
	...
	static public String commissionReport(CommissionReport commissionReport) {
		String msg = "commission report:" + " execId=" + commissionReport.m_execId + " commission="
				+ Util.DoubleMaxString(commissionReport.m_commission) + " currency=" + commissionReport.m_currency
				+ " realizedPNL=" + Util.DoubleMaxString(commissionReport.m_realizedPNL) + " yield="
	...
	static public String positionMulti(int reqId, String account, String modelCode, Contract contract, double pos,
			double avgCost) {
		String msg = " ---- Position begin ----\n" + "id = " + reqId + "\n" + "account = " + account + "\n"
	...
	static public String accountUpdateMulti(int reqId, String account, String modelCode, String key, String value,
			String currency) {
		String msg = " id = " + reqId + " account = " + account + " modelCode = " + modelCode + " key = " + key
	...
	public static String securityDefinitionOptionalParameter(int reqId, String exchange, int underlyingConId,
		...
		String msg = " id = " + reqId + "exchange = " + exchange + " underlyingConId = " + underlyingConId
				+ " tradingClass = " + tradingClass + " multiplier = " + multiplier + " expirations: " + expirationsStr
				+ " strikes: " + strikesStr;
