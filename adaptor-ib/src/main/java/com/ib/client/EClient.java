...
//    public EReader reader()             { return m_reader; }
	...
	protected void sendConnectRequest() throws IOException {
		...
		if (!m_useV100Plus || m_connectOptions == null) {
			send(CLIENT_VERSION); // Do not add length prefix here, because Server does not know Client's version
									// yet
		} else {
	...
	public synchronized void reqMktData(int tickerId, Contract contract, String genericTickList, boolean snapshot,
		...
		try {
			...
			if (m_serverVersion >= 31) {
				/*
				 * Note: Even though SHORTABLE tick type supported only starting server version
				 * 33 it would be relatively expensive to expose this restriction here.
				 *
	...
	/**
	 * Note that formatData parameter affects intra-day bars only; 1-day bars always
	 * return with date in YYYYMMDD format.
	 */
	...
	public synchronized void reqRealTimeBars(int tickerId, Contract contract, int barSize, String whatToShow,
			boolean useRTH, ArrayList<TagValue> realTimeBarsOptions) {
		// not connected?
		if (!isConnected()) {
			notConnected();
			return;
	...
	public synchronized void reqMktDepth(int tickerId, Contract contract, int numRows,
			ArrayList<TagValue> mktDepthOptions) {
		// not connected?
		if (!isConnected()) {
			notConnected();
			return;
	...
	public synchronized void placeOrder(int id, Contract contract, Order order) {
		...
		if (m_serverVersion < MIN_SERVER_VER_SCALE_ORDERS) {
			if (order.scaleInitLevelSize() != Integer.MAX_VALUE || order.scalePriceIncrement() != Double.MAX_VALUE) {
		...
		if (m_serverVersion < MIN_SERVER_VER_DELTA_NEUTRAL_CONID) {
			if (order.deltaNeutralConId() > 0 || !IsEmpty(order.deltaNeutralSettlingFirm())
					|| !IsEmpty(order.deltaNeutralClearingAccount()) || !IsEmpty(order.deltaNeutralClearingIntent())) {
		...
		if (m_serverVersion < MIN_SERVER_VER_DELTA_NEUTRAL_OPEN_CLOSE) {
			if (!IsEmpty(order.deltaNeutralOpenClose()) || order.deltaNeutralShortSale()
					|| order.deltaNeutralShortSaleSlot() > 0 || !IsEmpty(order.deltaNeutralDesignatedLocation())) {
				error(id, EClientErrors.UPDATE_TWS,
		...
		if (m_serverVersion < MIN_SERVER_VER_SCALE_ORDERS3) {
			if (order.scalePriceIncrement() > 0 && order.scalePriceIncrement() != Double.MAX_VALUE) {
				if (order.scalePriceAdjustValue() != Double.MAX_VALUE
						...
						|| order.scaleProfitOffset() != Double.MAX_VALUE || order.scaleAutoReset()
						|| order.scaleInitPosition() != Integer.MAX_VALUE
						|| order.scaleInitFillQty() != Integer.MAX_VALUE || order.scaleRandomPercent()) {
					error(id, EClientErrors.UPDATE_TWS,
		...
		if (m_serverVersion < MIN_SERVER_VER_ORDER_COMBO_LEGS_PRICE
				&& SecType.BAG.name().equalsIgnoreCase(contract.getSecType())) {
		...
		if (m_serverVersion < MIN_SERVER_VER_ORDER_SOLICITED) {
			if (order.solicited()) {
				error(id, EClientErrors.UPDATE_TWS, "  It does not support order solicited parameter.");
				return;
			}
		}
		...
		if (m_serverVersion < MIN_SERVER_VER_MODELS_SUPPORT) {
			if (!IsEmpty(order.modelCode())) {
				error(id, EClientErrors.UPDATE_TWS, "  It does not support model code parameter.");
				return;
			}
		}
		...
		try {
			...
			if (m_serverVersion >= 19) {
				...
				double lower = (m_serverVersion == 26 && order.getOrderType().equals("VOL")) ? Double.MAX_VALUE
						: order.stockRangeLower();
				double upper = (m_serverVersion == 26 && order.getOrderType().equals("VOL")) ? Double.MAX_VALUE
			...
			if (m_serverVersion >= 26) { // Volatility orders
				...
				if (m_serverVersion < 28) {
					...
					if (m_serverVersion >= MIN_SERVER_VER_DELTA_NEUTRAL_CONID
							&& !IsEmpty(order.getDeltaNeutralOrderType())) {
	...
	public synchronized void reqSecDefOptParams(int reqId, String underlyingSymbol, String futFopExchange,
