...
public class ComboLegDlg extends JDialog {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -6617192672737231972L;
	...
	public ComboLegDlg(ArrayList<ComboLeg> comboLegs, ArrayList<OrderComboLeg> orderComboLegs, String orderExchange,
			JDialog owner) {
		super(owner, true);
	...
	public void onAddLeg() {
		try {
			...
			m_comboLegsModel.addComboLeg(new ComboLeg(conId, ratio, m_action.getText(), m_exchange.getText(), openClose,
					shortSaleSlot, m_designatedLocation.getText(), exemptCode), new OrderComboLeg(price));
		} catch (Exception e) {
			reportError("Error - ", e);
			return;
		}
...
class ComboLegModel extends AbstractTableModel {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 6253110866143080008L;
