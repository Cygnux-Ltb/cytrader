...
public class OrderDlg extends JDialog {
	...
	private static void addGBComponent(IBGridBagPanel panel, Component comp, GridBagConstraints gbc, int weightx,
			int gridwidth) {
		gbc.weightx = weightx;
	...
	public OrderDlg(SampleFrame owner) {
		...
		addGBComponent(pContractDetails, new JLabel("Last trade date or contract month"), gbc, COL1_WIDTH,
		...
		addGBComponent(pOrderDetails, new JLabel("Lmt Price / Option Price / Stop Price / Volatility"), gbc, COL1_WIDTH,
				GridBagConstraints.RELATIVE);
		addGBComponent(pOrderDetails, m_lmtPrice, gbc, COL2_WIDTH, GridBagConstraints.REMAINDER);
		addGBComponent(pOrderDetails, new JLabel("Aux Price / Underlying Price"), gbc, COL1_WIDTH,
				GridBagConstraints.RELATIVE);
		...
		addGBComponent(pOptionsExercise, new JLabel("Number of Contracts"), gbc, COL1_WIDTH,
				GridBagConstraints.RELATIVE);
		...
		String dateTime = "" + gc.get(Calendar.YEAR) + pad(gc.get(Calendar.MONTH) + 1)
				+ pad(gc.get(Calendar.DAY_OF_MONTH)) + " " + pad(gc.get(Calendar.HOUR_OF_DAY)) + ":"
				+ pad(gc.get(Calendar.MINUTE)) + ":" + pad(gc.get(Calendar.SECOND)) + " "
		...
		addGBComponent(pBackfill, new JLabel("Bar Size Setting (1 to 11)"), gbc, COL1_WIDTH,
				GridBagConstraints.RELATIVE);
		...
		addGBComponent(pBackfill, new JLabel("Regular Trading Hours (1 or 0)"), gbc, COL1_WIDTH,
				GridBagConstraints.RELATIVE);
		addGBComponent(pBackfill, m_UseRTH, gbc, COL2_WIDTH, GridBagConstraints.REMAINDER);
		addGBComponent(pBackfill, new JLabel("Date Format Style (1 or 2)"), gbc, COL1_WIDTH,
				GridBagConstraints.RELATIVE);
	...
	protected void onBtnAdjStop() {
		showModalPanelDialog(new CallableWithParam() {
			@Override
	...
	void onAddComboLegs() {
		ComboLegDlg comboLegDlg = new ComboLegDlg(m_contract.comboLegs(), m_order.orderComboLegs(),
	...
	void onBtnOptions() {
		SmartComboRoutingParamsDlg smartComboRoutingParamsDlg = new SmartComboRoutingParamsDlg(m_optionsDlgTitle,
				m_options, this);
