...
public class SampleRfq extends SimpleWrapper {
	...
	private static final int MaskRecvAll = MaskBidPrice | MaskBidSize | MaskAskPrice | MaskAskSize;
	...
	public void testOrder() throws Exception {
		...
		if (client() != null && client().isConnected()) {
			...
			if (m_status == Status.Done) {
				String msg = "Done, bid=" + m_bidSize + "@" + m_bidPrice + " ask=" + m_askSize + "@" + m_askPrice;

				DeltaNeutralContract underComp = m_contract.underComp();
				if (underComp != null) {
					msg += " DN: conId=" + underComp.conid() + " price=" + underComp.price() + " delta="
							+ underComp.delta();
				}
				consoleMsg(msg);
			}
		...
		try {
			synchronized (m_mutex) {
				if (m_status == Status.SecDef) {
					...
					ComboLeg comboLeg = new ComboLeg(contractDetails.contract().conid(), /* ratio */ 1,
					...
					if (m_contract.underComp() != null) {
						...
						if (m_needFrontMonthFuture) {
							...
							Contract futContract = new FutContract(contractDetails.contract().symbol(),
									/* all expirations */ "", contractDetails.contract().currency());
	...
	public void tickPrice(int tickerId, int field, double price, int canAutoExecute) {
		...
		consoleMsg("tickPrice:" + tickerId + " field:" + field + " (" + tick.field() + ") value:" + price);
	...
	public void tickSize(int tickerId, int field, int size) {
		TickType tick = TickType.get(field);
		consoleMsg("tickSize:" + tickerId + " field:" + field + " (" + tick.field() + ") value:" + size);
