...
public interface IOrderBook extends WriteBytesMarshallable, StateHash {
	...
	/**
	 * Process new order. Depending on price specified (whether the order is
	 * marketable), order will be matched to existing opposite GTC orders from the
	 * order book. In case of remaining volume (order was not matched completely):
	 * IOC - reject it as partially filled. GTC - place as a new limit order into th
	 * order book.
	 * <p>
	 ...
	 * Move order
	 * <p>
	 * newPrice - new price (if 0 or same - order will not moved) fills cmd.action
	 * with original original order action
	 *
	 * @param cmd - order command
	 * @return MATCHING_UNKNOWN_ORDER_ID if order was not found, otherwise SUCCESS
	 */
	 ...
	 * <p>
	 * Slow, because order book do not maintain uid-to-order index.
	 * <p>
	 * Produces garbage.
	 * <p>
	 * Orders must be processed before doing any other mutable call.
	 * <p>
	 *
	 * @param uid user id
	...
	/**
	 * State hash for order books is implementation-agnostic Look
	 * {@link IOrderBook#validateInternalState} for full internal state validation
	 * for de-serialized objects
	 *
	...
	default int stateHash() {
		...
		// log.debug(" Ask orders stream: {}",
		// orderBook.askOrdersStream(true).collect(Collectors.toList()));
		// log.debug(" Ask orders hash: {}",
		// stateHashStream(orderBook.askOrdersStream(true)));
		// log.debug(" Bid orders stream: {}",
		// orderBook.bidOrdersStream(true).collect(Collectors.toList()));
		// log.debug(" Bid orders hash: {}",
		...
		return Objects.hash(HashingUtils.stateHashStream(askOrdersStream(true)),
				HashingUtils.stateHashStream(bidOrdersStream(true)), getSymbolSpec().stateHash());
	}
