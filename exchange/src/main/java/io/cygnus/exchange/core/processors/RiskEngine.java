...
public final class RiskEngine implements WriteBytesMarshallable {
	...
	public RiskEngine(final int shardId, final long numShards, final ISerializationProcessor serializationProcessor,
		...
		if (exchangeConfiguration.getInitStateCfg().fromSnapshot()) {
			...
			final State state = serializationProcessor.loadData(exchangeConfiguration.getInitStateCfg().getSnapshotId(),
					ISerializationProcessor.SerializedModuleType.RISK_ENGINE, shardId, bytesIn -> {
						...
						final SymbolSpecificationProvider symbolSpecificationProvider = new SymbolSpecificationProvider(
								bytesIn);
			...
			this.binaryCommandsProcessor = new BinaryCommandsProcessor(this::handleBinaryMessage,
					this::handleReportQuery, sharedPool, exchangeConfiguration.getReportsQueriesCfg(), shardId);
	...
	/**
	 * Pre-process command handler 1. MOVE/CANCEL commands ignored, for specific uid
	 * marked as valid for matching engine 2. PLACE ORDER checked with risk ending
	 * for specific uid 3. ADD USER, BALANCE_ADJUSTMENT processed for specific uid,
	 * not valid for matching engine 4. BINARY_DATA commands processed for ANY uid
	 * and marked as valid for matching engine TODO which handler marks? 5. RESET
	 * commands processed for any uid
	 *
	 * @param cmd - command
	...
	private CommandResultCode adjustBalance(long uid, int currency, long amountDiff, long fundingTransactionId,
			BalanceAdjustmentType adjustmentType) {
		final CommandResultCode res = userProfileService.balanceAdjustment(uid, currency, amountDiff,
		...
		if (cmd.action == OrderAction.BID) {
			...
			if (cmd.orderType == OrderType.FOK_BUDGET || cmd.orderType == OrderType.IOC_BUDGET) {
				...
				if (cmd.reserveBidPrice != cmd.price) {
					// log.warn("reserveBidPrice={} less than price={}", cmd.reserveBidPrice,
					// cmd.price);
...
//            log.warn("orderAmount={} > userProfile.accounts.get({})={}", orderAmount, currency, userProfile.accounts.get(currency));
	 ...
	 * <p>
	 * NOTE: Current implementation does not care about accounts and positions
...
//                newRequiredMarginForSymbol, position.currency, userProfile.accounts.get(position.currency), freeMargin);
		...
		// check if current balance and margin can cover new required margin for symbol
		// position
	...
	public boolean handlerRiskRelease(final long seq, final OrderCommand cmd) {
		...
		if (mte != null && mte.eventType != MatcherEventType.BINARY_EVENT) {
			...
			if (spec.type == SymbolType.CURRENCY_EXCHANGE_PAIR) {
				...
				final SymbolPositionRecord takerSpr = (takerUp != null) ? takerUp.getPositionRecordOrThrowEx(symbol)
						: null;
				do {
		...
		if (marketData != null && cfgMarginTradingEnabled) {
			final RiskEngine.LastPriceCacheRecord record = lastPriceCache.getIfAbsentPut(symbol,
					RiskEngine.LastPriceCacheRecord::new);
	...
	private void handleMatcherEventMargin(final MatcherTradeEvent ev, final CoreSymbolSpecification spec,
			final OrderAction takerAction, final UserProfile takerUp, final SymbolPositionRecord takerSpr) {
		if (takerUp != null) {
	...
	private void handleMatcherRejectReduceEventExchange(final OrderCommand cmd, final MatcherTradeEvent ev,
			final CoreSymbolSpecification spec, final boolean takerSell, final UserProfile taker) {
	...
	private void handleMatcherEventsExchangeSell(MatcherTradeEvent ev, final CoreSymbolSpecification spec,
		...
		if (taker != null) {
			taker.accounts.addToValue(quoteCurrency,
	...
	private void handleMatcherEventsExchangeBuy(MatcherTradeEvent ev, final CoreSymbolSpecification spec,
			final UserProfile taker, final OrderCommand cmd) {
