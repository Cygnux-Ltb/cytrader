...
//                || cmd.command == OrderCommandType.BINARY_DATA_QUERY) {
	...
	private <T extends ApiCommand> CompletableFuture<CommandResultCode> submitCommandAsync(
			EventTranslatorOneArg<OrderCommand, T> translator, final T apiCommand) {
		return submitCommandAsync(translator, apiCommand, c -> c.resultCode);
	}

	private <T extends ApiCommand> CompletableFuture<OrderCommand> submitCommandAsyncFullResponse(
			EventTranslatorOneArg<OrderCommand, T> translator, final T apiCommand) {
...
//                log.debug("seq={} cmd.size={} data={}", seq, cmd.size, cmd.price);
	...
	private void publishPersistCmd(final ApiPersistState api, final LongLongConsumer seqConsumer) {
		...
		try {
			...
			// log.debug("seq={} cmd.command={} data={}", firstSeq, cmdMatching.command,
			// cmdMatching.price);
	...
	private static final EventTranslatorOneArg<OrderCommand, ApiCancelOrder> CANCEL_ORDER_TRANSLATOR = (cmd, seq,
			api) -> {
	...
	private static final EventTranslatorOneArg<OrderCommand, ApiReduceOrder> REDUCE_ORDER_TRANSLATOR = (cmd, seq,
			api) -> {
	...
	private static final EventTranslatorOneArg<OrderCommand, ApiOrderBookRequest> ORDER_BOOK_REQUEST_TRANSLATOR = (cmd,
			seq, api) -> {
	...
	private static final EventTranslatorOneArg<OrderCommand, ApiSuspendUser> SUSPEND_USER_TRANSLATOR = (cmd, seq,
			api) -> {
	...
	private static final EventTranslatorOneArg<OrderCommand, ApiResumeUser> RESUME_USER_TRANSLATOR = (cmd, seq,
			api) -> {
...
//            log.debug("REPLAY seq={} cmd={}", seq, cmd);
	...
	public long placeNewOrder(int userCookie, long price, long reservedBidPrice, long size, OrderAction action,
	...
	public void placeNewOrder(int serviceFlags, long eventsGroup, long timestampNs, long orderId, int userCookie,
			long price, long reservedBidPrice, long size, OrderAction action, OrderType orderType, int symbol,
			long uid) {
