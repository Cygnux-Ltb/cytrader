...
//        }
	...
	/**
	 * Suspend removes inactive clients profile from the core in order to increase
	 * performance. Account balances should be first adjusted to zero with
	 * BalanceAdjustmentType=SUSPEND. No open margin positions allowed in the
	 * suspended profile. However in some cases profile can come back with positions
	 * and non-zero balances, if pending orders or pending commands was not
	 * processed yet. Therefore resume operation must be able to merge profile.
	 *
	 * @param uid client id
	 * @return result code
	 */
	...
	public CommandResultCode resumeUserProfile(long uid) {
		...
		if (userProfile == null) {
			...
			// resume existing suspended profile (can contain non empty positions or
			// accounts)
