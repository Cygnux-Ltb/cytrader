...
public final class TwoStepMasterProcessor implements EventProcessor {
	...
	public TwoStepMasterProcessor(final RingBuffer<OrderCommand> ringBuffer, final SequenceBarrier sequenceBarrier,
			final SimpleEventHandler eventHandler, final ExceptionHandler<OrderCommand> exceptionHandler,
			final CoreWaitStrategy coreWaitStrategy, final String name) {
	 ...
	 *
	 * @throws IllegalStateException if this object instance is already running in a
	 *                               thread
	 */
	@Override
	public void run() {
				...
				if (nextSequence <= availableSequence) {
					while (nextSequence <= availableSequence) {
						...
						if (cmd.command == OrderCommandType.SHUTDOWN_SIGNAL) {
							// having all sequences aligned with the ringbuffer cursor is a requirement for
							// proper shutdown
