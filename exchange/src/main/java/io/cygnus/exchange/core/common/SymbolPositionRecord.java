...
//        }
	...
	public long estimateProfit(final CoreSymbolSpecification spec,
			final RiskEngine.LastPriceCacheRecord lastPriceCacheRecord) {
		switch (direction) {
		case EMPTY:
			return profit;
		case LONG:
	 ...
	 * @return -1 if order will reduce current exposure (no additional margin
	 *         required), otherwise full margin for symbol position if order
	 *         placed/executed
	 */
	...
	/**
	 * Update position for one user 1. Un-hold pending size 2. Reduce opposite
	 * position accordingly (if exists) 3. Increase forward position accordingly (if
	 * size left in the trading event)
	 *
	 * @param action order action
	 * @param size   order size
	...
	private long closeCurrentPositionFutures(final OrderAction action, final long tradeSize, final long tradePrice) {
		...
		// log.debug("{} {} {} {} cur:{}-{} profit={}", uid, action, tradeSize,
		// tradePrice, position, totalSize, profit);
		...
		// current position smaller than trade size, can close completely and calculate
		// profit
	...
	public void reset() {
		...
		// log.debug("records: {}, Pending B{} S{} total size: {}", records.size(),
		// pendingBuySize, pendingSellSize, totalSize);
	...
	public int stateHash() {
		return Objects.hash(symbol, currency, direction.getMultiplier(), openVolume, openPriceSum, profit,
				pendingSellSize, pendingBuySize);
	}

	@Override
	public String toString() {
		return "SPR{" + "u" + uid + " sym" + symbol + " cur" + currency + " pos" + direction + " Σv=" + openVolume
				+ " Σp=" + openPriceSum + " pnl=" + profit + " pendingS=" + pendingSellSize + " pendingB="
				+ pendingBuySize + '}';
	}
}
