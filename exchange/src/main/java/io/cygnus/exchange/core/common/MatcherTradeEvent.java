...
public final class MatcherTradeEvent {
	...
	// TRADE, REDUCE, REJECT (rare) or BINARY_EVENT (reports data)
	...
	// actual price of the deal (from maker order), 0 for rejection (price can be
	// take from original order)
	public long price;
...
//        evt.timestamp = this.timestamp;
	...
	public boolean equals(Object o) {
		...
		if (o == null)
			return false;
		if (!(o instanceof MatcherTradeEvent))
			return false;
		MatcherTradeEvent other = (MatcherTradeEvent) o;

		// ignore timestamp
		return section == other.section && activeOrderCompleted == other.activeOrderCompleted
				&& matchedOrderId == other.matchedOrderId && matchedOrderUid == other.matchedOrderUid
				&& matchedOrderCompleted == other.matchedOrderCompleted && price == other.price && size == other.size
				&& bidderHoldPrice == other.bidderHoldPrice && ((nextEvent == null && other.nextEvent == null)
	...
	@Override
	public int hashCode() {
		return Objects.hash(section, activeOrderCompleted, matchedOrderId, matchedOrderUid, matchedOrderCompleted,
				price, size, bidderHoldPrice, nextEvent);
	}
