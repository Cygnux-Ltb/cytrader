...
public final class SerializationUtils {
	...
	public static long[] bytesToLongArrayLz4(final LZ4Compressor lz4Compressor, final NativeBytes<Void> bytes,
			final int padding) {
		...
		final ByteBuffer byteBufferCompressed = ByteBuffer
				.allocate(4 + lz4Compressor.maxCompressedLength(originalSize));
		...
		return toLongsArray(byteBufferCompressed.array(), byteBufferCompressed.arrayOffset(), compressedBytesLen,
				padding);
	}
	...
	public static Wire longsLz4ToWire(long[] dataArray, int longsTransfered) {
		...
		lz4FastDecompressor.decompress(byteBuffer, byteBuffer.position(), uncompressedByteBuffer,
				uncompressedByteBuffer.position(), originalSizeBytes);
	...
	public static <T> LongObjectHashMap<T> readLongHashMap(final BytesIn<?> bytes,
			final Function<BytesIn<?>, T> creator) {
		int length = bytes.readInt();
	...
	public static <T> IntObjectHashMap<T> readIntHashMap(final BytesIn<?> bytes,
			final Function<BytesIn<?>, T> creator) {
		int length = bytes.readInt();
	...
	public static <T extends WriteBytesMarshallable> void marshallLongMap(final Map<Long, T> map,
			final BytesOut<?> bytes) {
	...
	public static <T, M extends Map<Long, T>> M readLongMap(final BytesIn<?> bytes, final Supplier<M> mapSupplier,
			final Function<BytesIn<?>, T> creator) {
		int length = bytes.readInt();
	...
	public static <K, V> void marshallGenericMap(final Map<K, V> map, final BytesOut<?> bytes,
			final BiConsumer<BytesOut<?>, K> keyMarshaller, final BiConsumer<BytesOut<?>, V> valMarshaller) {
		bytes.writeInt(map.size());
	...
	public static <K, V, M extends Map<K, V>> M readGenericMap(final BytesIn<?> bytes, final Supplier<M> mapSupplier,
			final Function<BytesIn<?>, K> keyCreator, final Function<BytesIn<?>, V> valCreator) {
		int length = bytes.readInt();
		final M map = mapSupplier.get();
		for (int i = 0; i < length; i++) {
	...
	public static <T> void marshallNullable(final T object, final BytesOut<?> bytes,
			final BiConsumer<T, BytesOut<?>> marshaller) {
