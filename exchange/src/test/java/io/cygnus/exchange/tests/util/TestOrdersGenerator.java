...
public final class TestOrdersGenerator {
	...
	public static MultiSymbolGenResult generateMultipleSymbols(final TestOrdersGeneratorConfig config) {
		...
		final List<CoreSymbolSpecification> coreSymbolSpecifications = config.getCoreSymbolSpecifications();
		...
		try (ExecutionTime ignore = new ExecutionTime(t -> log.debug("All test commands generated in {}", t))) {
			...
			final LongConsumer sharedProgressLogger = createAsyncProgressLogger(
			...
			for (int i = coreSymbolSpecifications.size() - 1; i >= 0; i--) {
				...
				final int commandsNum = (i != 0) ? (int) (totalTransactionsNumber * distribution[i] + 0.5)
						: Math.max(quotaLeft, 1);
...
//                log.debug("{}. Generating symbol {} : commands={} orderBookSizeTarget={} (quotaLeft={})", i, spec.symbolId, commandsNum, orderBookSizeTarget, quotaLeft);
				futures.put(spec.symbolId, CompletableFuture.supplyAsync(() -> {
					final int[] uidsAvailableForSymbol = UserCurrencyAccountsGenerator
							.createUserListForSymbol(usersAccounts, spec, commandsNum);
					final int numUsers = uidsAvailableForSymbol.length;
					final UnaryOperator<Integer> uidMapper = idx -> uidsAvailableForSymbol[idx];
					return generateCommands(commandsNum, orderBookSizeTarget, numUsers, uidMapper, spec.symbolId, false,
							config.isAvalancheIOC(), sharedProgressLogger, seed);
				}));
			}
		...
		final int benchmarkCmdSize = genResults.values().stream()
				.mapToInt(genResult -> genResult.commandsBenchmark.size()).sum();

		final CompletableFuture<List<ApiCommand>> apiCommandsFill = mergeCommands(genResults, config.getSeed(), false,
				CompletableFuture.completedFuture(null));

		final CompletableFuture<List<ApiCommand>> apiCommandsBenchmark = mergeCommands(genResults, config.getSeed(),
				true, apiCommandsFill);

		return MultiSymbolGenResult.builder().genResults(genResults).apiCommandsFill(apiCommandsFill)
				.apiCommandsBenchmark(apiCommandsBenchmark).benchmarkCommandsSize(benchmarkCmdSize).build();
	}

	private static CompletableFuture<List<ApiCommand>> mergeCommands(Map<Integer, GenResult> genResults, long seed,
			boolean takeBenchmark, CompletableFuture<?> runAfterThis) {
		...
		log.debug("Merging {} commands for {} symbols ({})...", commandsLists.stream().mapToInt(Collection::size).sum(),
				genResults.size(), takeBenchmark ? "benchmark" : "preFill");

		final List<OrderCommand> merged = RandomCollectionsMerger.mergeCollections(commandsLists, seed);

		final CompletableFuture<List<ApiCommand>> resultFuture = runAfterThis
				.thenApplyAsync(ignore -> TestOrdersGenerator.convertToApiCommand(merged));
...
//        Arrays.stream(doubles).sorted().forEach(d -> log.debug("{}", d));
	...
	public static GenResult generateCommands(final int benchmarkTransactionsNumber, final int targetOrderBookOrders,
			final int numUsers, final UnaryOperator<Integer> uidMapper, final int symbol,
			final boolean enableSlidingPrice, final boolean avalancheIOC, final LongConsumer asyncProgressConsumer,
			final int seed) {
...
//        final IOrderBook orderBook = new OrderBookDirectImpl(SYMBOLSPEC_EUR_USD, ObjectsPool.createDefaultTestPool());
		...
		final TestOrdersGeneratorSession session = new TestOrdersGeneratorSession(orderBook,
				benchmarkTransactionsNumber, targetOrderBookOrders / 2, // asks + bids
		...
		return GenResult.builder().commandsBenchmark(commandsBenchmark).commandsFill(commandsFill)
				.finalOrderbookHash(orderBook.stateHash()).finalOrderBookSnapshot(l2MarketData).build();
	}
	...
	private static void updateOrderBookSizeStat(TestOrdersGeneratorSession session) {
		...
		// log.debug("ask={}, bif={} seq={} filledAtSeq={}", ordersNumAsk, ordersNumBid,
		// session.seq, session.filledAtSeq);
...
//                log.debug("{}", dumpOrderBook(l2MarketDataSnapshot));
	...
	private static void matcherTradeEventEventHandler(final TestOrdersGeneratorSession session,
			final MatcherTradeEvent ev, final OrderCommand orderCommand) {
	...
	private static OrderCommand generateRandomOrder(TestOrdersGeneratorSession session) {
		...
		final boolean requireFastFill = session.filledAtSeq == null
				|| lackOfOrders > session.lackOrOrdersFastFillThreshold;

		final boolean growOrders = lackOfOrders > 0;

		// log.debug("{} growOrders={} requireFastFill={} lackOfOrders({})={}",
		// session.seq, growOrders, requireFastFill, action, lackOfOrders);

		if (session.filledAtSeq == null && !growOrders) {
			session.filledAtSeq = session.seq;
			// log.debug("Symbol {} filled at {} (targetOb={} trans={})", session.symbol,
			// session.seq, session.targetOrderBookOrdersHalf, session.transactionsNumber);
		}
...
//        session.hdrRecorder.recordValue(Math.min(System.nanoTime() - t, Integer.MAX_VALUE));
		...
		if (q == 2) {
			...
			// log.debug("session.seq={} orderId={} size={} p={}", session.seq, orderId,
			// session.actualOrders.size(), priceMoveRounded);
	...
	private static OrderCommand generateRandomGtcOrder(TestOrdersGeneratorSession session) {
		...
		return OrderCommand.builder().command(OrderCommandType.PLACE_ORDER).uid(uid).orderId(newOrderId).action(action)
				.orderType(OrderType.GTC).size(size).price(price)
	...
	private static OrderCommand generateRandomInstantOrder(TestOrdersGeneratorSession session) {
		...
		if (session.avalancheIOC) {
			...
			reserveBidPrice = action == OrderAction.BID ? session.maxPrice : 0; // set limit price
			final long availableVolume = action == OrderAction.ASK ? session.lastTotalVolumeAsk
...
//                    log.debug("huge size={} at {}", placeCmd.size, session.seq);
		...
		return OrderCommand.builder().command(OrderCommandType.PLACE_ORDER).orderType(orderType).uid(uid)
				.orderId(newOrderId).action(action).size(size).price(priceOrBudget).reserveBidPrice(reserveBidPrice)
				.build();
	}
	...
	public static List<ApiCommand> convertToApiCommand(List<OrderCommand> commands, int from, int to) {
		try (ExecutionTime ignore = new ExecutionTime(
				t -> log.debug("Converted {} commands to API commands in: {}", to - from, t))) {
			ArrayList<ApiCommand> apiCommands = new ArrayList<>(to - from);
			for (int i = from; i < to; i++) {
				final OrderCommand cmd = commands.get(i);
				switch (cmd.command) {
				case PLACE_ORDER:
					apiCommands.add(ApiPlaceOrder.builder().symbol(cmd.symbol).uid(cmd.uid).orderId(cmd.orderId)
							.price(cmd.price).size(cmd.size).action(cmd.action).orderType(cmd.orderType)
							.reservePrice(cmd.reserveBidPrice).build());
					break;
	...
	private static void printStatistics(final List<OrderCommand> allCommands) {
		...
		final String commandsFokb = String.format("%.2f%%",
				(float) counterPlaceFOKB / (float) commandsListSize * 100.0f);
		final String commandsCancel = String.format("%.2f%%",
				(float) counterCancel / (float) commandsListSize * 100.0f);
		final String commandsMove = String.format("%.2f%%", (float) counterMove / (float) commandsListSize * 100.0f);
		final String commandsReduce = String.format("%.2f%%",
				(float) counterReduce / (float) commandsListSize * 100.0f);
		log.info("GTC:{} IOC:{} FOKB:{} cancel:{} move:{} reduce:{}", commandsGtc, commandsIoc, commandsFokb,
				commandsCancel, commandsMove, commandsReduce);

		final String cpsMax = String.format("%d (%.2f%%)", symbolStat.getMax(),
				symbolStat.getMax() * 100.0f / commandsListSize);
		final String cpsAvg = String.format("%d (%.2f%%)", (int) symbolStat.getAverage(),
				symbolStat.getAverage() * 100.0f / commandsListSize);
		final String cpsMin = String.format("%d (%.2f%%)", symbolStat.getMin(),
				symbolStat.getMin() * 100.0f / commandsListSize);
