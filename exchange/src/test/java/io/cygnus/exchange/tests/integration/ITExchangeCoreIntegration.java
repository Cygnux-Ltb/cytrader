...
public abstract class ITExchangeCoreIntegration {
	...
	private void basicFullCycleTest(final CoreSymbolSpecification symbolSpec) {
		...
		try (final ExchangeTestContainer container = ExchangeTestContainer.create(getPerformanceConfiguration())) {
			...
			final ApiPlaceOrder order101 = ApiPlaceOrder.builder().uid(UID_1).orderId(101).price(1600).size(7)
					.action(ASK).orderType(GTC).symbol(symbolSpec.symbolId).build();
			...
			final ApiPlaceOrder order102 = ApiPlaceOrder.builder().uid(UID_1).orderId(102).price(1550)
					.reservePrice(reserve102).size(4).action(OrderAction.BID).orderType(GTC).symbol(symbolSpec.symbolId)
					.build();
			...
			final ApiPlaceOrder order201 = ApiPlaceOrder.builder().uid(UID_2).orderId(201).price(1700)
					.reservePrice(reserve201).size(2).action(OrderAction.BID).orderType(OrderType.IOC)
			...
			final ApiPlaceOrder order202 = ApiPlaceOrder.builder().uid(UID_2).orderId(202).price(1583)
					.reservePrice(reserve202).size(4).action(OrderAction.BID).orderType(GTC).symbol(symbolSpec.symbolId)
					.build();
			...
			// ### 4. first trader moves his order - it will match existing order (202) but
			// not entirely
			final ApiMoveOrder moveOrder = ApiMoveOrder.builder().symbol(symbolSpec.symbolId).uid(UID_1).orderId(101)
					.newPrice(1580).build();
	...
	public void exchangeRiskBasicTest() throws Exception {
		...
		try (final ExchangeTestContainer container = ExchangeTestContainer.create(getPerformanceConfiguration())) {
			...
			// try submit an order - limit BUY 7 lots, price 300K satoshi (30K x10 step) for
			// each lot 100K szabo
			// should be rejected
			final ApiPlaceOrder order101 = ApiPlaceOrder.builder().uid(UID_1).orderId(101).price(30_000)
					.reservePrice(30_000).size(7).action(OrderAction.BID).orderType(GTC).symbol(SYMBOL_EXCHANGE)
					.build();
			...
			// add 100K more
			container.submitCommandSync(ApiAdjustUserBalance.builder().uid(UID_1).currency(CURRENECY_XBT)
					.amount(100_000).transactionId(223948217349827L).build(), CHECK_SUCCESS);
			...
			// verify order placed with correct reserve price and account balance is updated
			// accordingly
			container.validateUserState(UID_1, profile -> {
			...
			// try submit an order - sell 7 lots, price 300K satoshi (30K x10 step) for each
			// lot 100K szabo
			// should be rejected
			final ApiPlaceOrder order102 = ApiPlaceOrder.builder().uid(UID_2).orderId(102).price(30_000).size(7)
					.action(ASK).orderType(OrderType.IOC).symbol(SYMBOL_EXCHANGE).build();
			container.submitCommandSync(order102, cmd -> {
				assertThat(cmd.resultCode, is(CommandResultCode.RISK_NSF));
			});
			...
			// add 1 szabo more
			container.submitCommandSync(ApiAdjustUserBalance.builder().uid(UID_2).currency(CURRENECY_ETH).amount(1)
					.transactionId(2193842938742L).build(), CHECK_SUCCESS);
	...
	public void exchangeRiskMoveTest() throws Exception {
		...
		try (final ExchangeTestContainer container = ExchangeTestContainer.create(getPerformanceConfiguration())) {
			...
			// each lot 100K szabo
			// should be rejected
			container.submitCommandSync(ApiPlaceOrder.builder().uid(UID_1).orderId(202).price(30_000).size(1001)
					.action(ASK).orderType(GTC).symbol(SYMBOL_EXCHANGE).build(), cmd -> {
						assertThat(cmd.resultCode, is(CommandResultCode.RISK_NSF));
					});
			...
			// submit order again - should be placed
			container.submitCommandSync(ApiPlaceOrder.builder().uid(UID_1).orderId(202).price(30_000).size(1000)
					.action(ASK).orderType(GTC).symbol(SYMBOL_EXCHANGE).build(), cmd -> {
						assertThat(cmd.resultCode, is(CommandResultCode.SUCCESS));
			...
			container.submitCommandSync(
					ApiPlaceOrder.builder().uid(UID_2).orderId(203).price(18_000).reservePrice(19_000).size(500)
							.action(OrderAction.BID).orderType(GTC).symbol(SYMBOL_EXCHANGE).build(),
					cmd -> {
						assertThat(cmd.resultCode, is(CommandResultCode.RISK_NSF));
					});
			...
			container.submitCommandSync(
					ApiPlaceOrder.builder().uid(UID_2).orderId(203).price(18_000).reservePrice(18_500).size(500)
							.action(OrderAction.BID).orderType(GTC).symbol(SYMBOL_EXCHANGE).build(),
					cmd -> {
						assertThat(cmd.resultCode, is(CommandResultCode.SUCCESS));
						assertThat(cmd.command, is(OrderCommandType.PLACE_ORDER));
			...
			container.submitCommandSync(
					...
					cmd -> {
						...
						assertThat(evt.bidderHoldPrice, is(18_500L)); // user original reserve price from bidder order
																		// (203)
			...
			container.submitCommandSync(
					ApiCancelOrder.builder().orderId(202).uid(UID_1).symbol(SYMBOL_EXCHANGE).build(), cmd -> {
						assertThat(cmd.resultCode, is(CommandResultCode.SUCCESS));
	...
	public void exchangeCancelBid() throws Exception {
		...
		try (final ExchangeTestContainer container = ExchangeTestContainer.create(getPerformanceConfiguration())) {
			...
			// cancel remaining order
			container.submitCommandSync(
					ApiCancelOrder.builder().orderId(203).uid(UID_2).symbol(SYMBOL_EXCHANGE).build(), cmd -> {
						assertThat(cmd.resultCode, is(CommandResultCode.SUCCESS));
						assertThat(cmd.command, is(OrderCommandType.CANCEL_ORDER));
						assertThat(cmd.orderId, is(203L));
						assertThat(cmd.uid, is(UID_2));
