...
public final class UserCurrencyAccountsGenerator {
	...
	/**
	 * Generates random users and different currencies they should have, so the
	 * total account is between accountsToCreate and
	...
	public static List<BitSet> generateUsers(final int accountsToCreate, Collection<Integer> currencies) {
		...
		do {
			...
			final int accountsToOpen = Math.min(Math.min(1 + (int) paretoDistribution.sample(), currencyCodes.length),
					totalAccountsQuota);
...
//            log.debug("{}", bitSet);
		...
		log.debug("Generated {} users with {} accounts up to {} different currencies in {}", result.size(),
				accountsToCreate, currencies.size(), executionTime.getTimeFormatted());
		return result;
	}

	public static int[] createUserListForSymbol(final List<BitSet> users2currencies, final CoreSymbolSpecification spec,
			int symbolMessagesExpected) {

		// we would prefer to choose from same number of users as number of messages to
		// be generated in tests
