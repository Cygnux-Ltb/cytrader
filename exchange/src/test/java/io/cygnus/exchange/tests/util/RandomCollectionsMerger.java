...
public class RandomCollectionsMerger {
	...
	public static <T> ArrayList<T> mergeCollections(@Nonnull final Collection<? extends Collection<T>> chunks,
			final long seed) {

		log.info("chunks.size()=={}, seed=={}", chunks.size(), seed);
		...
		List<Pair<Spliterator<T>, Double>> weightPairs = chunks.stream()
				.map(chunk -> Pair.create(chunk.spliterator(), (double) chunk.size())).collect(Collectors.toList());

		while (!weightPairs.isEmpty()) {

			final EnumeratedDistribution<Spliterator<T>> ed = new EnumeratedDistribution<>(jdkRandomGenerator,
					weightPairs);
			...
			weightPairs = weightPairs.stream().filter(p -> p.getFirst().estimateSize() > 0)
