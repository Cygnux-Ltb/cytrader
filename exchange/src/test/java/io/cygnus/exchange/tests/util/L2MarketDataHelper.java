...
public class L2MarketDataHelper {
	...
	public L2MarketDataHelper(L2MarketData l2) {
		log.info("Construct L2MarketDataHelper...");
	...
	public String dumpOrderBook(L2MarketData l2MarketData) {
		...
		s.append(".").append(Strings.repeat("-", priceWith - 2)).append("ASKS").append(Strings.repeat("-", volWith - 1))
				.append(".\n");
		...
		s.append("|").append(Strings.repeat("-", priceWith)).append("+").append(Strings.repeat("-", volWith))
				.append("|\n");
		...
		s.append("'").append(Strings.repeat("-", priceWith - 2)).append("BIDS").append(Strings.repeat("-", volWith - 1))
				.append("'\n");
		return s.toString();
	}

	private static int maxWidth(int minWidth, long[]... arrays) {
		return Arrays.stream(arrays).flatMapToLong(Arrays::stream).mapToInt(p -> String.valueOf(p).length()).max()
				.orElse(minWidth);
	}

	@SuppressWarnings("unused")
	private static int maxWidth(int minWidth, int[]... arrays) {
		return Arrays.stream(arrays).flatMapToInt(Arrays::stream).map(p -> String.valueOf(p).length()).max()
				.orElse(minWidth);
	}
